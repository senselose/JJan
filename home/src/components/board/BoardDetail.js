...
import FavoriteIcon from '@mui/icons-material/Favorite'; // 좋아요 아이콘 import
import FavoriteBorderIcon from '@mui/icons-material/FavoriteBorder'; // 좋아요 아이콘 (비어있는 버전)
...
const BoardDetail = () => {
  ...
  const fetchBoard = useCallback(async () => {
    try {
      ...
      setLikes(response.data.boardLikes || 0); // 좋아요 수 설정
      setLiked(response.data.liked); // 사용자가 이미 좋아요를 눌렀는지 여부 설정
      setLoading(false);

      // 조회수 증가 함수 호출
      await BoardService.incrementViews(id);
    } catch (error) {
    ...
    <Container>
      <Paper className="board-detail-container">
        ...
        <Box display="flex" alignItems="center">
          ...
          <Typography variant="body2" color="textSecondary" style={{ marginLeft: '8px' }}>{likes}</Typography> {/* 좋아요 수 표시 */}
        </Box>
      </Paper>
